...
class GroupingAutoencoder(nn.Module):
    def __init__(self, encoder, decoder, grouping_weight=0.3, margin=0.5):
        super().__init__()
    ...
    def compute_losses(self, x):
        ...
        # normalize embeddings for the grouping loss only, so the
        # network can't just inflate embedding magnitude to game
        # the distance terms. decoder still uses the raw z.
        z_norm = F.normalize(z, p=2, dim=1)
        z_pos_norm = F.normalize(z_pos, p=2, dim=1)
        z_neg_norm = F.normalize(z_neg, p=2, dim=1)

        d_pos = torch.sum((z_norm - z_pos_norm) ** 2, dim=1)
        d_neg = torch.sum((z_norm - z_neg_norm) ** 2, dim=1)
